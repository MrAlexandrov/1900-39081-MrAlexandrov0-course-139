...
public:
  ...
  std::string HandleRequestThrow(
    ...
    if (userResult.IsEmpty()) {
      ...
      return userver::formats::json::ToString(userver::formats::json::MakeObject("error", "user with this email is not registered"));
    }
    ...
    if (password != user.password) {
      auto &response = request.GetHttpResponse();
      response.SetStatus(userver::server::http::HttpStatus::kNotFound);
      return userver::formats::json::ToString(userver::formats::json::MakeObject("error", "wrong password"));
    }
    ...
    if (result.IsEmpty()) {
      ...
      return userver::formats::json::ToString(userver::formats::json::MakeObject("error", "failed to create session"));
    }
